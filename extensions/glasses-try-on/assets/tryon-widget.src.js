/**
 * This is the source file — edit THIS one, not tryon-widget.js.
 *
 * Shopify enforces a 10,000 byte limit on a theme app extension block's
 * declared JS asset (AssetSizeAppBlockJavaScript) — this file, mostly
 * comments, is well over that (confirmed on a real deploy: the CLI still
 * reported "success" but the oversized asset 404'd on Shopify's CDN,
 * silently breaking the storefront widget with no error anywhere obvious).
 * `tryon-widget.js` (what tryon-button.liquid/tryon-embed.liquid actually
 * reference) is a minified build of this file — regenerated automatically
 * by `npm run deploy` (see package.json's `predeploy` script), or by hand
 * via `npm run build:widget`. Never edit tryon-widget.js directly; it gets
